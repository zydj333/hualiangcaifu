...
class Role extends Admin_Controller {
    ...
    $seldata['rolename']=$rolename;
		...
		$this->cismarty->display('rights/role/role_list.html');
	...
	function role_edit()
	{
		if(isset($_POST['dosubmit'])){
		  ...
		  $this->a_system_model->update(array('id'=>$_data['id']),$_data,$this->tb_role);
			...
			$this->cismarty->display('rights/role/role_edit.html');
		}
	}
	...
	function role_add()
	{
		if(isset($_POST['dosubmit'])){
			...
			$this->cismarty->display('rights/role/role_add.html');
		}
	}
	...
	function role_menu()
	{
		if(isset($_POST['dosubmit'])){
			...
			$this->load->model('a_system_model');
			$this->a_system_model->del(array('role_id'=>$_data['id']),$this->tb_role_priv);
		    $ids=$_data['hdnids'];
				$arr_ids=	explode(',',$ids);
			...
			$menulist=$this->a_system_model->get_query(array('isshow'=>1,'order_by'=>'listorder asc'),$this->tb_menu);
			...
			$this->cismarty->assign('info',$info);
			$this->cismarty->display('rights/role/role_menu.html');
		}
	}
