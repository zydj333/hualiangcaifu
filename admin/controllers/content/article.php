...
class Article extends Admin_Controller {
    ...
    public function lists(){
		...
		$ac_list = $this->article_category->get_query(array('ac_code' => 'news','order_by'=>'listorder desc'),'article_category');
    	...
    	$this->cismarty->display('content/article/article_lists.html');
    }
	//文章添加
	public function article_add(){
	    if(isset($_POST['dosubmit'])){
			...
			$article['isrecommend'] = $this->input->post('isrecommend');
			...
			if($_FILES['article_focus']['error'] == 0){
			    /***保存图片***/
			    $this->user_id=$this->session->userdata('admin_user_id');
			    $this->load->library('iupload_lib');
			    $config=array(
		      ...
		      $this->iupload_lib->initialize($config);//配置初始化文件
	        $this->iupload_lib->do_uploadfile('article_focus');//上传附件
			    $file_id=$this->iupload_lib->save_data();//保存数据到数据库
			    if($file_id == TRUE){
		      		$link_pic = $this->article->get_one(array('file_id'=>$file_id),'uploadfile','filepath');
		      		$article['article_focus'] = $link_pic['filepath'];
			    }
			}
			...
			$this->showmessage('article_add',lang('com_add'),$this->admin_url.'content/article/lists?loghash='.$this->session->userdata('loghash'));
		}else{
		    ...
		    $this->cismarty->display('content/article/article_add.html');
		}
	    ...
	    if(isset($_POST['dosubmit'])){
	        ...
	        $this->iupload_lib->do_uploadfile('article_focus');//上传附件
			    ...
			    if($file_id == TRUE){
		            $link_pic = $this->article->get_one(array('file_id'=>$file_id),'uploadfile','filepath');
		            $article['article_focus'] = $link_pic['filepath'];
		            $this->article->del(array('rel_id'=>$article_id,'shop_id'=>0,'module'=>'article'),'uploadfile_rel');
		            $this->article->add(array('file_id'=>$file_id,'rel_id'=>$article_id,'shop_id'=>0,'module'=>'article'),'uploadfile_rel');
			    }
			}
			...
			$this->showmessage('success',lang('com_edit'),$this->admin_url.'content/article/lists?loghash='.$this->session->userdata('loghash'));
		}else{
		    ...
		    $ac_list = $this->article_category->get_query(array('order_by'=>'listorder desc','ac_code' => 'news'),'article_category');
			foreach($ac_list as $key=>$val){
				$ac_list[$key]['id']=$val['ac_id'];
				$ac_list[$key]['name']=$val['ac_name'];
			}
			...
			$this->cismarty->display('content/article/article_edit.html');
		}
	}
	//文章删除
	public function article_del(){
		...
		$this->showmessage('article',lang('com_del'),$this->admin_url.'content/article/lists?loghash='.$this->session->userdata('loghash'));
	}
	//文章排序
	public function article_order(){
	    $lc_id = $this->input->post('id');
		    ...
		    $this->article->update(array('article_id' => $lc_id[$key]),$data);
			}
    ...
    public function article_category(){
		...
		$list=$this->article_category->get_query(array('ac_code'=>'news','order_by'=>'listorder desc'));
		...
		$this->cismarty->assign('codetype','news');
		$this->cismarty->assign('infolist',$gc_list);
    	$this->cismarty->display('content/article/article_category.html');
    }
    ...
    public function article_category_cache(){
		...
		$where = array('order_by'=>'listorder desc,ac_id desc');
	...
	public function ajax_category_class(){
		...
		$where = array('parent_id' => $gc_parent_id,'ac_code' => $gc_code,'order_by'=>'listorder desc');
	...
	public function article_category_add(){
	    if(isset($_POST['dosubmit'])){
			...
			if($code_type == 'news'){
				$this->showmessage('article_category',lang('com_add'),$this->admin_url.'content/article/article_category?loghash='.$this->session->userdata('loghash'));
	    	}else{
	    		$this->showmessage('article_category',lang('com_add'),$this->admin_url.'content/article/help_category?loghash='.$this->session->userdata('loghash'));
	    	}
		}else{
	        ...
	        $this->cismarty->display('content/article/article_category_add.html');
		}
	}
	//分类编辑
	public function article_category_edit(){
	    if(isset($_POST['dosubmit'])){
		    ...
		    $this->cismarty->display('content/article/article_category_edit.html');
		}
	}
	//分类删除
	public function article_category_del(){
	    $_data_post=$this->input->post();
		$ac_id=isset($_data_post['check_ac_id'])?$_data_post['check_ac_id']:null;
		if($ac_id !== false){
		...
		$this->showmessage('article_category',lang('com_del'),$this->admin_url.'content/article/article_category?loghash='.$this->session->userdata('loghash'));
	}
	//分类排序
	public function article_category_order(){
		...
		$this->showmessage('article_category',lang('com_order'),$this->admin_url.'content/article/article_category?loghash='.$this->session->userdata('loghash'));
	}
	//AJAX判断分类名称是否存在
	...
	//图片删除
	public function ajax_article_pic_del(){
	  $file_id=$this->input->get('file_id');
	  $up_res = $this->article->get_one(array('file_id' => $file_id),'uploadfile');
	  $pic = empty($up_res['filepath']) ? '' : $up_res['filepath'];
		@unlink($_SERVER['DOCUMENT_ROOT'].$pic);
		$this->load->model('uploadfile');
		$this->uploadfile->del(array('file_id' => $file_id));
		$pic = true;
