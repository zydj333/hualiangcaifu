...
class Link extends Admin_Controller{
	...
	*/
	...
	public function link_list(){
	    ...
	    $this->cismarty->display('marketing/link/link_list.html');
	}
	//添加
...
//			}
			$this->showmessage('success',lang('com_add'),$this->admin_url.'marketing/link/link_list?loghash='.$this->session->userdata('loghash'));
		}else{
			$this->load->model('a_system_model');
		    $lc_list = $this->a_system_model->get_query(array('order_by'=>'lc_id asc'),$this->tb_link_category);
			$this->cismarty->assign('lc_list',$lc_list);
		    $this->cismarty->display('marketing/link/link_add.html');
		}
	}
	//编辑
	public function link_edit(){
		...
		if(isset($_POST['dosubmit'])){
			...
			$this->showmessage('link_list',lang('com_edit'),$this->admin_url.'marketing/link/link_list?loghash='.$this->session->userdata('loghash'));

		}else{
			$link_id=intval($link_id);
			$this->load->model('a_system_model');
		    $link = $this->a_system_model->get_one(array('link_id'=>$link_id),$this->tb_link);
		    if(empty($link)) $this->showmessage('error',lang('com_parameter'),$this->admin_url.'marketing/link/link_list?loghash='.$this->session->userdata('loghash'));
		    $lc_list = $this->a_system_model->get_query(array('order_by'=>'lc_id asc'),$this->tb_link_category);
		    ...
		    $this->cismarty->display('marketing/link/link_edit.html');
		}
	}
	 //删除
    public function link_del(){
		...
		$this->showmessage('link_del',lang('com_del'),$this->admin_url.'marketing/link/link_list?loghash='.$this->session->userdata('loghash'));
	}
	/**
	 * 友情链接类型管理
	 */
	public function link_category_manage(){
		...
		if(!empty($op)){
			...
			$this->showmessage('error',lang('com_parameter'),$this->admin_url.'marketing/link/link_list?loghash='.$this->session->userdata('loghash'));
		}
	}
	//类型管理
	private function link_category_list(){
		...
		$this->cismarty->display('marketing/link/link_category_manage.html');
	}
	/***
	 * 删除友情链接分类
	 */
	private function link_category_del(){
		...
		$this->showmessage('success',lang('com_del'),$this->admin_url.'marketing/link/link_category_manage?op=list&loghash='.$this->session->userdata('loghash'));
	}
	 /***
	 * 添加友情链接分类
	 */
	private function link_category_add(){
		...
		if($dosubmit == 'ok'){
			...
			if(empty($_data_post['lc_name']) || empty($_data_post['listorder']))
				$this->showmessage('error',lang('com_parameter'),$this->admin_url.'marketing/link/link_category_manage?op=add&loghash='.$this->session->userdata('loghash'));
			...
			$this->showmessage('adddialog',lang('com_add'),$this->admin_url.'marketing/link/link_category_manage?op=list&loghash='.$this->session->userdata('loghash'));
		}else{
		    ...
		    $this->cismarty->display('marketing/link/link_category.html');
		}
	}
	/***
	 * 编辑友情链接分类
	*/
	private function link_category_edit(){
		...
		if($dosubmit == 'ok'){
			...
			$this->showmessage('editdialog',lang('com_edit'),$this->admin_url.'marketing/link/link_category_manage?op=list&loghash='.$this->session->userdata('loghash'));
		}else{
			$lc_id = $this->input->get('id');
		    $lc_id=intval($lc_id);
			$link_category = $this->link_category_model->get_one(array('lc_id'=>$lc_id),'link_category');
	...
	private function link_category_order(){
	  $lc_id = $this->input->post('hdnid');
		...
		foreach($listorder as $key => $val){
		    ...
		    $this->link_category_model->update(array('lc_id' => $lc_id[$key]),$data,'link_category');
		}
	...
	public function link_order(){
		...
		$this->showmessage('link_list',lang('com_order'),$this->admin_url.'marketing/link/link_list?loghash='.$this->session->userdata('loghash'));
	}
