...
class Buyconsult extends Admin_Controller {
	...
	public function lists(){
		...
		$list=$this->a_member_model->get_buyconsult_search($data);

		$this->cismarty->assign('sel',$data);
		$this->cismarty->assign('infolist',$list['list']);
		$this->cismarty->assign('infopage',$list['page']);
		$this->cismarty->display('member/buyconsult/consult_lists.html');
	}
	...
	public function del(){
		...
		if(is_array($consult_id)){
			foreach($consult_id as $v){
				$this->order_model->del(array('consult_id'=>$v),'consult_buy');
			}
		}else{
			$consult_id=intval($consult_id);
			$this->order_model->del(array('consult_id'=>$consult_id),'consult_buy');
		}
	...
	public function consult_reply(){
		if(isset($_POST['dosubmit'])){
			...
			if($consult_id>0){
				$info=$this->a_member_model->get_one(array('consult_id'=>$consult_id),'consult_buy');
				if(!empty($info)){
					...
					if (empty($_data['consult_reply'])) {
						...
						$this->a_member_model->update(array('consult_id'=>$consult_id),$_data,'consult_buy');
    		...
    		if($consult_id>0){
				...
				if(!empty($info)){
					...
					$this->cismarty->display('member/buyconsult/consult_reply.html');
					return;
				}
			}
			$this->showmessage('error',lang('com_parameter'),$this->admin_url.'member/buyconsult/lists?loghash='.$this->session->userdata('loghash'));
    	}
	}
