...
class Order extends Admin_Controller {
	...
	*/
    ...
    $seldata['pagesize']=10;
		...
		$list=$this->a_caifu_model->get_order_search($seldata);
	  ...
	  $this->cismarty->display('caifu/order/order_list.html');
	}
	...
	public function order_view(){
		...
		if(isset($_POST['dosubmit'])){
		  ...
		  if(empty($order)) $this->showmessage('error',lang('com_parameter'),$this->admin_url.'caifu/order/lists?loghash='.$this->session->userdata('loghash'));
			$user_id = $order['user_id'];
			$product_id = $order['product_id'];
			$user_info = $this->a_caifu_model->get_one(array('user_id'=>$user_id), 'user');
			$product_info = $this->a_caifu_model->get_one(array('id'=>$product_id),'caifu_product');
		  ...
		  $this->cismarty->display('caifu/order/order_info.html');
		}
	}
	 //删除
    public function order_del(){
	    $_data_post=$this->input->post();
			...
			foreach($order_id as $val){
				$order = $this->a_caifu_model->get_one(array('id'=>$val),$this->tb_order);
				$image = explode('.',$order['money_slip']);
				@unlink($_SERVER['DOCUMENT_ROOT'].'/'.$image[0].'_w'.'.'.$image[1]);
				@unlink($_SERVER['DOCUMENT_ROOT'].'/'.$order['money_slip']);
				$where = array('id' => $val);
				$return = $this->a_caifu_model->del($where,$this->tb_order,$val);
			}

			$this->showmessage('link_del',lang('com_del'),$this->admin_url.'caifu/order/lists?loghash='.$this->session->userdata('loghash'));
	}
