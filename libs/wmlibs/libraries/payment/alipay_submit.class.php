...
class AlipaySubmit {
	...
	function buildRequestForm($para_temp, $method, $button_name) {
	 ...
	 /**
     * 建立请求，以表单HTML形式构造（Button提交）
     * @param $para_temp 请求参数数组
     * @param $method 提交方式。两个值可选：post、get
     * @param $button_name 确认按钮显示文字
     * @return 提交表单HTML文本
     */
	function buildRequestButton($para_temp, $method, $button_name) {
		//待请求参数数组
		$para = $this->buildRequestPara($para_temp);
		...
		//submit按钮控件请不要含有name属性
        $sHtml = $sHtml."<a href='javascript:void(0);' onclick=\"$('#alipaysubmit').submit();\" class='cart-button'>".$button_name."</a></form>";
     ...
     * return 时间戳字符串
	 */
	function query_timestamp_pay_by() {
