...
class iupfile_lib
{
	var $saveName;// 保存名
	var $savePath;// 保存路径
	var $fileFormat = array('gif','GIF','jpg','JPG','doc','DOC','rar','zip','doc','pdf','flv','wmv','application/octet-stream');// 文件格式&MIME限定
...
// @param $overwriet 是否覆盖 1 允许覆盖 0 禁止覆盖
	function doUpload($savePath, $fileFormat='',$maxSize = 0, $overwrite = 0) {
...
// @param $fileArray 文件信息数组
	function copyfile($fileArray){
		...
		$this->returninfo['savepath'] = $this->savePath.$this->saveName;
