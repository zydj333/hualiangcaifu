...
class CI_Upload {
	...
	// Is $_FILES[$field] set? If not, no reason to continue.
		...
		//Is the user space allowed to be uploaded?
		if(!$this->is_allowed_remain_space_size())
		{
			$this->set_error('upload_remain_space');
			return FALSE;
		}
	...
	public function is_allowed_filetype($ignore_mime = FALSE)
	{
		...
		{
			$file_temp_size= getimagesize($this->file_temp);
			if ($file_temp_size === FALSE)
			{
				return FALSE;
			}
			$this->file_type=isset($file_temp_size['mime'])?$file_temp_size['mime']:$this->file_type;
		}
	...
	/**
	 * Verify that the file is within the allowed remain_space_size
	 *
	 * @return	bool
	 */
	public function is_allowed_remain_space_size()
	{
		if ($this->remain_space_size > 0  AND  $this->file_size > $this->remain_space_size)
		{
			return FALSE;
		}
		else
	...
	public function set_error($msg)
	{
		...
		if (is_array($msg))
		{
			foreach ($msg as $val)
			{
				...
				$this->error_msg[] = $msg;
				$this->error_msg_mark[] = $val;
				log_message('error', $msg);
			}
		}
		else
		{
			$this->error_msg_mark[] = $msg;
