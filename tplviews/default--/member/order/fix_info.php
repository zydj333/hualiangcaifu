...
<div class="mntopbg"><span class="info_navzi2">我的订单</span>
  ...
  <div>
	...
	<tr><td colspan="2" height="26" style="border-bottom:1px solid #dddddd; padding-left:15px; font-size:13px;"><b>返修信息</b></td></tr>
	<tr><td style="border-bottom:1px solid #dddddd; padding:10px 10px 10px 10px; color:#666666; line-height:22px;"><img src="<!--{IMG_PATH}-->proddpic2.jpg" style="vertical-align:middle;"/> <b>返修状态： <!--{$goods_info['state_name']}--></b></td></tr>
	<tr><td style="padding:10px 10px 10px 10px; color:#666666;">返修编号：<!--{$return_info['return_sn']}--></td></tr>
	<tr><td style="padding:10px 10px 10px 10px; color:#666666;">订单编号：<!--{$return_info['order_sn']}--></td></tr>
	
	</table>
	</td><td valign="middle" style="padding:0 0 0 20px; font-size:14px;">
		返修说明：<!--{$return_info['buyer_message']}--><br>
		<!--{if !empty($return_info['pic1'])}-->返修图片：<a href="<!--{$return_info['pic1']}-->" target="_blank">图片预览一</a><br><!--{/if}-->
		<!--{if !empty($return_info['pic2'])}-->返修图片：<a href="<!--{$return_info['pic2']}-->" target="_blank">图片预览二</a><br><!--{/if}-->
		<!--{if !empty($return_info['pic3'])}-->返修图片：<a href="<!--{$return_info['pic3']}-->" target="_blank">图片预览三</a><br><!--{/if}-->
		<!--{if !empty($return_info['pic4'])}-->返修图片：<a href="<!--{$return_info['pic4']}-->" target="_blank">图片预览四</a><br><!--{/if}-->
		<!--{if !empty($return_info['pic5'])}-->返修图片：<a href="<!--{$return_info['pic5']}-->" target="_blank">图片预览五</a><br><!--{/if}-->
		<!--{if !empty($return_info['return_message'])}-->审核意见:<!--{$return_info['return_message']}--><!--{/if}-->
	</td></tr>
  </table>
  </div>
