  ...
  <div class="main_con clearfix">
    <div class="main_con_left"> <img src="<!--{IMG_PATH}-->left_img.png" class="font_size fix div_block" /> <img src="<!--{IMG_PATH}-->news_img.jpg" class="font_size fix div_block" />
      <ul class="left_menu">
        ...
        <!--{foreach $data $v}-->
        <li <!--{if $article_info['ac_id'] == $v['ac_id']}-->class="current"<!--{/if}-->><a href="index.php?c=news&a=lists&cid=<!--{$v['ac_id']}-->"><!--{$v['ac_name']}--></a></li>
        <!--{/foreach}-->
       	<!--{/cc}-->
			</ul>
...
<div class="newdtime">发布时间：<!--{date('Y-m-d H:m:s', $article_info['article_time'])}-->  来源：双鸟机械  浏览次数：<!--{$article_info['dot']}-->次</div>
<div class="newdcon">
