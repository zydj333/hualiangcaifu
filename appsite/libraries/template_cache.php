...
final class template_cache {
    ...
    public function __construct()
    {
       ...
       // $this->cache_path = APPPATH.'views';
        $this->temp_path = FCPATH.'tplviews';
        $this->cache_path = APPPATH.'views';
    }
    ...
    public function template_compile($module, $template, $style = 'default') {
        ...
        $tplfile=$this->temp_path.DIRECTORY_SEPARATOR.$style.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.$template.'.php';
    ...
    public function template_parse($str) {
        ...
        $str = preg_replace ( "/\<\!\-\-\{template\s+(.+)\}\-\-\>/", "<?php include APPPATH.'views/'.template(\\1); ?>", $str );
...
*/
        ...
        $str = preg_replace ( "/\<\!\-\-\{cc:(\w+)\s+([^}]+)\}\-\-\>/ie", "self::pc_tag('$1','$2', '$0')", $str);
        $str = preg_replace ( "/\<\!\-\-\{\/cc\}\-\-\>/ie", "self::end_pc_tag()", $str);
    ...
    public static function pc_tag($op, $data, $html) {
        ...
        $arr = array('action','num','cache','urlrule', 'return', 'start','setpages');
        ...
        if (!empty($cache) && !isset($page)) {
            $str .= '$tag_cache_name = md5(implode(\'&\','.self::arr_to_html($datas).').\''.$tag_id.'\');if(!$'.$return.' = tpl_cache($tag_cache_name)){';
        }
        if (in_array($op,$tools)) {
            switch ($op) {
                case 'json':
                    ...
                    break;
				case 'xml':
						$str .= 'if(!isset($CI))$CI =& get_instance();$CI->load->helper(\'ixml\');';
						$str .= '$xml_data = @file_get_contents(\''.$datas['url'].'\');';
						$str .= '$'.$return.' = $CI->ixml->xml_unserialize($xml_data);';
					break;
       /*         case 'block':
       ...
       */
            ...
            if ( file_exists(APPPATH.'models'.DIRECTORY_SEPARATOR.$op.'_model.php')) {
                $str .= 'if(!isset($CI))$CI =& get_instance();$CI->load->model("'.$op.'_model");if (method_exists($CI->'.$op.'_model, \''.$action.'\')) {';
                ...
                if (isset($page)) {
                    ...
                    $datas['action'] = $action;
                   // $str .= '$'.$op.'_total = $CI->'.$op.'_model->count('.self::arr_to_html($datas).');';

                   // $str .= 'if($'.$op.'_total>$pagesize){ $pages = pages($'.$op.'_total, $page, $pagesize, $urlrule); } else { $pages="" ;}';
                }
                $str .= '$'.$return.' = $CI->'.$op.'_model->'.$action.'('.self::arr_to_html($datas).');';
                $str .= '}';

            }
        }
        if (!empty($cache) && !isset($page)) {
        	$action=(!isset($op) || empty($op)) ? '':$op;
            $str .= 'if(!empty($'.$return.')){tpl_setcache($tag_cache_name, $'.$return.',\''.$action.'\','.$cache.');}';
            $str .= '}';
        }
        return "<"."?php ".$str."?".">";
    }
    ...
    static private function end_pc_tag() {
        return '<?php if(defined(\'BASEPATH\') && !defined(\'HTML\')) {if(isset($data))unset($data);}?>';
    }
