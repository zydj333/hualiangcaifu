...
class iquery
{
	...
	public function __set($name,$value)
	{
		switch($name)
		{
			...
			case 'group':$this->sql['group'] = empty($value)?'':'GROUP BY '.$value;break;
			case 'having':$this->sql['having'] = empty($value)?'':'having '.$value;break;
			case 'order':$this->sql['order'] = empty($value)?'':'order by '.$value;break;
			case 'limit':$this->sql['limit'] = empty($value)?'':'limit '.$value;break;
			//case 'limit':$value == '' ? '' : ($this->sql['limit'] = 'limit '.$value);break;
            case 'page':$this->sql['page'] =is_int($value)?intval($value):''; break;
