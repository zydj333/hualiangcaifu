$(document).ready(function(){
	...
	$('img[ectype="flex"]').click(function(){
		...
		if(status == 'open'){
			...
			//ajax
			$.ajax({
				url: admin_url+'content/article/ajax_category_class/?code='+code+'&loghash='+loghash+'&parent_id='+id+'&rrr='+Math.random(),
				dataType: 'json',
				success: function(data){
					...
					for(var i = 0; i < data.length; i++){
						...
						//新增下级
						if(data[i].deep < 2){
							src += "<a class='btn-add-nofloat marginleft' href='"+admin_url+"content/article/article_category_add/?code="+code+"&loghash="+loghash+"&parent_id="+data[i].ac_id+"'><span>新增下级</span></a>";
						}
						src += "</td>";

						//操作
						src += "<td class='w84'>";
						src += "<a href='"+admin_url+"content/article/article_category_edit/?loghash="+loghash+"&ac_id="+data[i].ac_id+"'>编辑</a>";
						src += " | <a href=\"javascript:del('"+admin_url+"content/article/article_category_del/?loghash="+loghash+"','check_ac_id[]',"+data[i].ac_id+");\">删除</a>";
						src += "</td>";
						src += "</tr>";
					}
					//插入
