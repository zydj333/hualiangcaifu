$(function(){
	...
	TransTpl += "默认运费：";
	TransTpl += "<input class=\"input-text\" type=\"text\" aria-label=\"默认运费件数\" maxlength=\"4\" autocomplete=\"off\" data-field=\"start\" value=\"1\" name=\"default[TRANSTYPE][start]\">";
	TransTpl += "<ddunit>"+transport_unit+"</ddunit>内，";
	TransTpl += "<input class=\"input-text\" type=\"text\" aria-label=\"默认运费价格\" maxlength=\"6\" autocomplete=\"off\" value=\"\" name=\"default[TRANSTYPE][postage]\" data-field=\"postage\">";
	TransTpl += "元， 每增加";
	TransTpl += "<input class=\"input-text\" type=\"text\" aria-label=\"每加<ddunit>件</ddunit>\" maxlength=\"4\" autocomplete=\"off\" value=\"1\" data-field=\"plus\" name=\"default[TRANSTYPE][plus]\">";
	TransTpl += "<ddunit>"+transport_unit+"</ddunit>， 增加运费";
	TransTpl += "<input class=\"input-text\" type=\"text\" aria-label=\"加<ddunit>件</ddunit>运费\" maxlength=\"6\" autocomplete=\"off\" value=\"\" data-field=\"postageplus\" name=\"default[TRANSTYPE][postageplus]\">";
	TransTpl += "元";
	...
	RuleHead += "<tr>\n";
	RuleHead += "<th>运送到<\/th>\n";
	RuleHead += "<th>首<ddmsg>"+transport_msg+"</ddmsg>(<ddunit>"+transport_unit+"</ddunit>)<\/th>\n";
	RuleHead += "<th>首费(元)<\/th>\n";
	RuleHead += "<th>续<ddmsg>"+transport_msg+"</ddmsg>(<ddunit>"+transport_unit+"</ddunit>)<\/th>\n";
	RuleHead += "<th>续费(元)<\/th>\n";
	RuleHead += "<th>操作<\/th>\n";
	RuleHead += "<\/tr>\n";
	RuleHead += "<\/thead>\n";
	...
	RuleCell += "<\/td>\n";
	RuleCell += "<td>\n";
	RuleCell += "<input class=\"input-text\" type=\"text\" aria-label=\"首件\" maxlength=\"4\" autocomplete=\"off\" value=\"1\" data-field=\"start\" name=\"special[TRANSTYPE][CurNum][start]\">\n";
	RuleCell += "<\/td>\n";
	RuleCell += "<td>\n";
	RuleCell += "<input class=\"input-text\" type=\"text\" aria-label=\"首费\" maxlength=\"6\" autocomplete=\"off\" value=\"\" data-field=\"postage\" name=\"special[TRANSTYPE][CurNum][postage]\">\n";
	RuleCell += "<\/td>\n";
	RuleCell += "<td>\n";
	RuleCell += "<input class=\"input-text\" type=\"text\" aria-label=\"续件\" maxlength=\"4\" autocomplete=\"off\" value=\"1\" data-field=\"plus\" name=\"special[TRANSTYPE][CurNum][plus]\">\n";
	RuleCell += "<\/td>\n";
	RuleCell += "<td>\n";
	RuleCell += "<input class=\"input-text\" type=\"text\" aria-label=\"续费\" maxlength=\"6\" autocomplete=\"off\" value=\"\" data-field=\"postageplus\" name=\"special[TRANSTYPE][CurNum][postageplus]\">\n";
	RuleCell += "<\/td>\n";
	RuleCell += "<td>\n";
...
/*	选择完区域后，确定事件*/
	$('.J_Submit').click(function (){
		var CityText = '', CityText2 = '', CityValue = '' , CityValue1 = '';
...
//		SelectArea = new Array();
		...
		$('#J_CityList').find('.province-list').find('input[name="a_name"]').each(function(){
			if ($(this).attr('checked')){
				CityValue1 += $(this).val()+',';
			}
		});
        ...
        CityValue1 = CityValue1.replace(/(,*$)/g,'');

		//返回选择的文本内容
		if (CityText == '')CityText = '未添加地区';
		$(objCurlArea).find('.area-group>p').html(CityText);
		//返回选择的值到隐藏域
		$('input[name="areas['+curTransType+']['+curIndex.substring(1)+']"]').val(CityValue+'|||'+CityText+'|||'+CityValue1);
		//关闭弹出层与遮罩层
	    $("#dialog_areas").css('display','none');
	    $('.ks-ext-mask').css('display','none');
	    //清空check_num显示的数量
...
//	                if(target.getAttribute("id")==$(objPanel).attr('id'))return true;
	...
	$('input[data-field="postage"]').live('blur',function (){
		...
		$(this).val(oNum);
		if($(this)[0].className=='input-text input-error') $(this).removeClass('input-error');
