$(function(){
    ...
    /* 商品图片ajax上传 */
	var url = DEFAULT_BASE_URL + 'index.php?m=com&c=comupload&a=image_upload';
	...
	$('span[nctype="drop_image"]').unbind().click(function(){
		$(this).parents('li')
			.find('img:first').attr('src',DEFAULT_BASE_URL+"/uploadfile/common/loading.gif")
	...
	$('.goods_demo').unbind().ajaxContent({
		...
		loaderType:"img",
		loadingMsg:DEFAULT_BASE_URL+"/uploadfile/common/loading.gif",
		target:'#demo'
	});
...
{
	$('img[nctype="'+id+'"]').attr('src',DEFAULT_BASE_URL+"/uploadfile/common/loading.gif");
	$.ajaxFileUpload
	(
		{
			url:url,
			...
			success: function (data, status)
			{
				...
				}else
				{
					...
					$('input[nctype="'+id+'"]').val(data.filepath);
					$('img[nctype="'+id+'"]').attr('src',DEFAULT_BASE_URL+'/'+data.filepath);
				}
				$.getScript("./statics/default/js/store_goods.js");
			},
